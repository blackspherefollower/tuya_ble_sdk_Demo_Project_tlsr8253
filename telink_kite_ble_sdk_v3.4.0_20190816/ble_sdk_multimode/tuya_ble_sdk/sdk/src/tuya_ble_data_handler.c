...
void tuya_ble_commonData_rx_proc(uint8_t *buf,uint16_t len)
{
    ...
    if(ty_trsmitr_proc.version<2)  //鍗忚涓荤増鏈彿浣庝簬2锛屼笉瑙ｆ瀽锛岃繑鍥炪�
    {
    ...
    if(tuya_ble_current_para.sys_settings.bound_flag==1)//褰撳墠宸茬粦瀹氱姸鎬�
    {
    ...
    TUYA_BLE_LOG_HEXDUMP_DEBUG("decryped data",(uint8_t*)air_recv_packet.de_encrypt_buf,air_recv_packet.decrypt_buf_len);//瑙ｅ瘑鏁版嵁
    //鎸囦护鏁版嵁crc楠岃瘉
    ...
    if(current_sn<=tuya_ble_receive_sn)
    {
        ...
        tuya_ble_gap_disconnect();//SN閿欒锛屾柇寮�摑鐗欒繛鎺�
        tuya_ble_free(air_recv_packet.de_encrypt_buf);
        return;
    }
    else
    ...
    ble_evt_buffer[0] = current_encry_mode;     //棣栧瓧鑺傛嫹璐濆姞瀵嗘柟寮忥紝渚夸簬鍚庣画浣跨敤
...
/**
recv_data[0]涓哄姞瀵嗘柟寮忥紝1/2/3/4涓篠N
**/
...
uint8_t tuya_ble_commData_send(uint16_t cmd,uint32_t ack_sn,uint8_t *data,uint16_t len,uint8_t encryption_mode)
{
    ...
    //鍒╃敤send_data buffer缂撳瓨鏄庢枃鎸囦护鏁版嵁
