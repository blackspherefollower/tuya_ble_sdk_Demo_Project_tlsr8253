...
#if (TUYA_BLE_PLATFORM==TUYA_BLE_PLATFORM_NRF5X)
#include "app_log.h"

#define  TUYA_BLE_LOG(...)   app_log_d(__VA_ARGS__)

#define  TUYA_BLE_HEXDUMP    app_log_dumpHex

#elif (TUYA_BLE_PLATFORM==TUYA_BLE_PLATFORM_REALTEK)

#include "trace.h"

extern uint8_t get_args(const char *format);
#define  TUYA_BLE_LOG(fmt,...) \
{ \
...
void TUYA_BLE_HEXDUMP(const char *name, uint8_t width, uint8_t *buf, uint16_t size);

#else
#include "../../../components/tlsr/tuya_log.h"
#define TUYA_BLE_LOG(...)     //tuya_log_d(__VA_ARGS__)
#define TUYA_BLE_HEXDUMP(...) //tuya_log_dumpHex(__VA_ARGS__)
#endif
